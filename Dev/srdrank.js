...
d3.select("#ATEAM").on("click", ateam);
...
function make_checkboxes(divid,fields,variable,mb_func, callback) {
    ...
    var cspans = opts.enter().insert("span").attr("class","inputs").text(function(d) {return d[0];});
...
svg.attr("height",Math.round((sortedr.length+1)*3.3*em)); //resize svg to the list length, I think
...
//gtext = genter.append("text")
//    .attr("fill-opacity", 0)
//    .attr("class","barcontent label")
//    .attr("x","5")
//    .attr("y",'2em')
...
grects.transition("x").delay(500).duration(500).attr("opacity",0.3);
